...
% H[a b c d]; a：子矩阵纵维度；b：子矩阵横维度；c：子载波数量；d：用户数量
%% Set parameters
if nargin <= 6
    ch.fc = 28e9; % 频率
    ch.Nc = 6;    % 簇  8
    ch.Np = 8;   % 每个簇中的路径数量  10
    ch.max_BS = pi; % BS角度范围
    % ch.max_BS1 = pi * 2 / 3; % BS角度范围
    ch.max_BS1 = ch.max_BS; % BS角度范围
    ...
    ch.fs=BW;
    ch.tau_max = (N_carrier / 2) / ch.fs;  % 路径最大时延
end
...
switch (length(Nr) + length(Nt))
    case 2  % creat mmWave channel(ULA)
        ...
        for i = 1 : K
            ...
            %             phi = sort(phi, 2); % 簇中心对齐实验 18/08/18
            ...
            %             theta = sort(theta, 2); % 簇中心对齐实验 18/08/18
